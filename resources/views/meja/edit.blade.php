...
<div style="background: white; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); padding: 30px; max-width: 600px;">
    <form action="{{ route('meja.update', $meja->id) }}" method="POST">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" name="nomer_meja" value="{{ old('nomer_meja', $meja->nomer_meja) }}" 
        ...
        <div style="margin-bottom: 20px;">
            ...
            <select name="kursi" 
                ...
                <option value="2" {{ old('kursi', $meja->kursi) == '2' ? 'selected' : '' }}>2 Kursi</option>
                <option value="4" {{ old('kursi', $meja->kursi) == '4' ? 'selected' : '' }}>4 Kursi</option>
                <option value="8" {{ old('kursi', $meja->kursi) == '8' ? 'selected' : '' }}>8 Kursi</option>
            </select>
        </div>
